...
class Player(Entity):
    ...
    def __init__(self, position, scale, gameLoop):
        ...
        self.wrapCooldown = 0
        self.image = pygame.image.load_extended("resources/player.png").convert_alpha()
        self.rotation = 0
    ...
    def update(self, deltaTime):
        ...
        fireVec = self.getInput(pygame.K_RIGHT, pygame.K_LEFT, pygame.K_DOWN, pygame.K_UP)
        if fireVec != [0, 0]:
    ...
    def move(self, deltaTime):
        movementVector = self.getInput(pygame.K_d, pygame.K_a, pygame.K_s, pygame.K_w)
    ...
    def rotate(self, x, y):
        xIndex = int(x + 1)
        yIndex = int(y + 1)
        table = [[self.rotation, 90, self.rotation], [0, self.rotation, 180], [self.rotation, -90, self.rotation]]
        self.image = pygame.transform.rotate(self.image, table[xIndex][yIndex] - self.rotation)
